...
(function ($) {
  ...
  function SlickGrid(container, data, columns, options) {
    ...
    var uid = "slickgrid_" + crypto.randomUUID();
    ...
    function setColumns(columnDefinitions) {
      ...
      for (var j = 0; j < columns.length; j++) {
        var n= columns[j] = $.extend({}, columnDefaults, columns[j]);
        columnsById[n.id] = j;
        if (n.minWidth && n.width < n.minWidth) {
          n.width = n.minWidth;
        }
        if (n.maxWidth && n.width > n.maxWidth) {
          n.width = n.maxWidth;
        }
      }
    ...
    function renderRows(range) {
      ...
      var i, ii; // Declare variables in the broader scope
...
for (i = range.top, ii = range.bottom; i <= ii; i++) {
    ...
    counter_rows_rendered++;
      ...
      for (var i1 = 0, i2 = rows.length; i1 < i2; i1++) {
        rowsCache[rows[i1]].rowNode = parentNode.appendChild(x.firstChild);
      }
    ...
    function asyncPostProcessRows() {
      ...
      while (postProcessFromRow <= postProcessToRow) {
		...
		if (vScrollDir >= 0) 
		{
    		row = postProcessFromRow;
    		postProcessFromRow++;
		}
		else 
		{
			row = postProcessToRow;
    		postProcessToRow--;
		}
